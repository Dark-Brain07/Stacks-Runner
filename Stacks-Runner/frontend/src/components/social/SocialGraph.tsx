...
export const SocialGraph = memo(forwardRef<HTMLDivElement, SocialGraphProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-socialgraph', `sr-socialgraph--${variant}`, `sr-socialgraph--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
