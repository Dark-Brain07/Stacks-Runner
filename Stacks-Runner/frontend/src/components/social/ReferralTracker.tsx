...
export const ReferralTracker = memo(forwardRef<HTMLDivElement, ReferralTrackerProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-referraltracker', `sr-referraltracker--${variant}`, `sr-referraltracker--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
