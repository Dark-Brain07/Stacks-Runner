...
export const Navbar = memo(forwardRef<HTMLDivElement, NavbarProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-navbar', `sr-navbar--${variant}`, `sr-navbar--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
