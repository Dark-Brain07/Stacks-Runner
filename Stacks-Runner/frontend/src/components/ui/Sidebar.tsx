...
export const Sidebar = memo(forwardRef<HTMLDivElement, SidebarProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-sidebar', `sr-sidebar--${variant}`, `sr-sidebar--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
