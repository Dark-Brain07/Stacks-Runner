...
export const Stepper = memo(forwardRef<HTMLDivElement, StepperProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-stepper', `sr-stepper--${variant}`, `sr-stepper--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
