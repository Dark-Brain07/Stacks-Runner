...
/**
 * ObstacleFactory - Game engine subsystem
 * @module services/ObstacleFactory
 */
export interface ObstacleFactoryOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class ObstacleFactory extends EventEmitter {
  private opts: ObstacleFactoryOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<ObstacleFactoryOptions> = {}) {
    super();
