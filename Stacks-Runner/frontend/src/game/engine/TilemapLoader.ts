...
/**
 * TilemapLoader - Game engine subsystem
 * @module services/TilemapLoader
 */
export interface TilemapLoaderOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class TilemapLoader extends EventEmitter {
  private opts: TilemapLoaderOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<TilemapLoaderOptions> = {}) {
    super();
