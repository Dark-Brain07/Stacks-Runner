...
/**
 * SoundMixer - Game engine subsystem
 * @module services/SoundMixer
 */
export interface SoundMixerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class SoundMixer extends EventEmitter {
  private opts: SoundMixerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<SoundMixerOptions> = {}) {
    super();
