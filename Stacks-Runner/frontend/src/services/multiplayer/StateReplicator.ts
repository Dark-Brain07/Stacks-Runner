...
/**
 * StateReplicator - Multiplayer networking service
 * @module services/StateReplicator
 */
export interface StateReplicatorOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class StateReplicator extends EventEmitter {
  private opts: StateReplicatorOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<StateReplicatorOptions> = {}) {
    super();
