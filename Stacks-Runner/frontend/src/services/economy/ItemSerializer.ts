...
/**
 * ItemSerializer - Economy and inventory management
 * @module services/ItemSerializer
 */
export interface ItemSerializerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class ItemSerializer extends EventEmitter {
  private opts: ItemSerializerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<ItemSerializerOptions> = {}) {
    super();
