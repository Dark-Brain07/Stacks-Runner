...
/**
 * CompletionChecker - Quest and progression service
 * @module services/CompletionChecker
 */
export interface CompletionCheckerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class CompletionChecker extends EventEmitter {
  private opts: CompletionCheckerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<CompletionCheckerOptions> = {}) {
    super();
