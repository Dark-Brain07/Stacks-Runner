...
/**
 * TimedChallenge - Quest and progression service
 * @module services/TimedChallenge
 */
export interface TimedChallengeOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class TimedChallenge extends EventEmitter {
  private opts: TimedChallengeOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<TimedChallengeOptions> = {}) {
    super();
