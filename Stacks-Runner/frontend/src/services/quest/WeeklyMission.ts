...
/**
 * WeeklyMission - Quest and progression service
 * @module services/WeeklyMission
 */
export interface WeeklyMissionOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class WeeklyMission extends EventEmitter {
  private opts: WeeklyMissionOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<WeeklyMissionOptions> = {}) {
    super();
