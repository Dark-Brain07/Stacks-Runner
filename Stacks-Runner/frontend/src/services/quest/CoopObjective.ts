...
/**
 * CoopObjective - Quest and progression service
 * @module services/CoopObjective
 */
export interface CoopObjectiveOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class CoopObjective extends EventEmitter {
  private opts: CoopObjectiveOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<CoopObjectiveOptions> = {}) {
    super();
