...
/**
 * RoleChecker - Security and authentication module
 * @module services/RoleChecker
 */
export interface RoleCheckerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class RoleChecker extends EventEmitter {
  private opts: RoleCheckerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<RoleCheckerOptions> = {}) {
    super();
