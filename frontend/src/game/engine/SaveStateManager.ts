...
/**
 * SaveStateManager - Game engine subsystem
 * @module services/SaveStateManager
 */
export interface SaveStateManagerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class SaveStateManager extends EventEmitter {
  private opts: SaveStateManagerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<SaveStateManagerOptions> = {}) {
    super();
