...
/**
 * PowerUpSpawner - Game engine subsystem
 * @module services/PowerUpSpawner
 */
export interface PowerUpSpawnerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class PowerUpSpawner extends EventEmitter {
  private opts: PowerUpSpawnerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<PowerUpSpawnerOptions> = {}) {
    super();
