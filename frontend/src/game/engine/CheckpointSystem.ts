...
/**
 * CheckpointSystem - Game engine subsystem
 * @module services/CheckpointSystem
 */
export interface CheckpointSystemOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class CheckpointSystem extends EventEmitter {
  private opts: CheckpointSystemOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<CheckpointSystemOptions> = {}) {
    super();
