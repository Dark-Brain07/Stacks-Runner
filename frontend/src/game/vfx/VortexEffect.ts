...
/**
 * VortexEffect - Visual effect renderer
 * @module services/VortexEffect
 */
export interface VortexEffectOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class VortexEffect extends EventEmitter {
  private opts: VortexEffectOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<VortexEffectOptions> = {}) {
    super();
