...
/**
 * AchievementClient - API client for backend communication
 * @module services/AchievementClient
 */
export interface AchievementClientOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class AchievementClient extends EventEmitter {
  private opts: AchievementClientOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<AchievementClientOptions> = {}) {
    super();
