...
export function createSamplingPipeline<T>(...fns: Fn<T, T>[]): Fn<T, T> {
  return (input: T) => fns.reduce((v, fn) => fn(v), input);
}

export async function createSamplingAsyncPipeline<T>(...fns: AsyncFn<T, T>[]): AsyncFn<T, T> {
...
export function samplingCache<K, V>(maxSize = 256): { get(k: K): V | undefined; set(k: K, v: V): void; clear(): void } {
