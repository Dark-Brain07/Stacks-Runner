...
export const ProfileViewer = memo(forwardRef<HTMLDivElement, ProfileViewerProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-profileviewer', `sr-profileviewer--${variant}`, `sr-profileviewer--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
