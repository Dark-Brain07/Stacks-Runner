...
export const BlockList = memo(forwardRef<HTMLDivElement, BlockListProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-blocklist', `sr-blocklist--${variant}`, `sr-blocklist--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
