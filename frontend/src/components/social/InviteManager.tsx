...
export const InviteManager = memo(forwardRef<HTMLDivElement, InviteManagerProps>(
  ({ className = '', variant = 'default', size = 'md', disabled, onClick, children }, ref) => {
    const cls = ['sr-invitemanager', `sr-invitemanager--${variant}`, `sr-invitemanager--${size}`, disabled ? 'sr-disabled' : '', className].filter(Boolean).join(' ');
    return (
