...
/**
 * XPCalculator - Quest and progression service
 * @module services/XPCalculator
 */
export interface XPCalculatorOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class XPCalculator extends EventEmitter {
  private opts: XPCalculatorOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<XPCalculatorOptions> = {}) {
    super();
