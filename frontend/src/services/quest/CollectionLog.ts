...
/**
 * CollectionLog - Quest and progression service
 * @module services/CollectionLog
 */
export interface CollectionLogOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class CollectionLog extends EventEmitter {
  private opts: CollectionLogOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<CollectionLogOptions> = {}) {
    super();
