...
/**
 * SeasonalEvent - Quest and progression service
 * @module services/SeasonalEvent
 */
export interface SeasonalEventOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class SeasonalEvent extends EventEmitter {
  private opts: SeasonalEventOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<SeasonalEventOptions> = {}) {
    super();
