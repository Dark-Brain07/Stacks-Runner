...
/**
 * GuildQuest - Quest and progression service
 * @module services/GuildQuest
 */
export interface GuildQuestOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class GuildQuest extends EventEmitter {
  private opts: GuildQuestOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<GuildQuestOptions> = {}) {
    super();
