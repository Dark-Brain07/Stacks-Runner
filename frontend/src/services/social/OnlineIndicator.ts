...
/**
 * OnlineIndicator - Social feature service
 * @module services/OnlineIndicator
 */
export interface OnlineIndicatorOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class OnlineIndicator extends EventEmitter {
  private opts: OnlineIndicatorOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<OnlineIndicatorOptions> = {}) {
    super();
