...
/**
 * MuteManager - Social feature service
 * @module services/MuteManager
 */
export interface MuteManagerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class MuteManager extends EventEmitter {
  private opts: MuteManagerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<MuteManagerOptions> = {}) {
    super();
