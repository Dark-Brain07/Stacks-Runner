...
/**
 * SpectatorService - Multiplayer networking service
 * @module services/SpectatorService
 */
export interface SpectatorServiceOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class SpectatorService extends EventEmitter {
  private opts: SpectatorServiceOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<SpectatorServiceOptions> = {}) {
    super();
