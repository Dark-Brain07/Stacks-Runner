...
/**
 * PacketSerializer - Multiplayer networking service
 * @module services/PacketSerializer
 */
export interface PacketSerializerOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class PacketSerializer extends EventEmitter {
  private opts: PacketSerializerOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<PacketSerializerOptions> = {}) {
    super();
