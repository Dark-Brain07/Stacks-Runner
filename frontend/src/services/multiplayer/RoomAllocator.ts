...
/**
 * RoomAllocator - Multiplayer networking service
 * @module services/RoomAllocator
 */
export interface RoomAllocatorOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class RoomAllocator extends EventEmitter {
  private opts: RoomAllocatorOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<RoomAllocatorOptions> = {}) {
    super();
