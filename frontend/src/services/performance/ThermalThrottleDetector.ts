...
/**
 * ThermalThrottleDetector - Performance monitoring module
 * @module services/ThermalThrottleDetector
 */
export interface ThermalThrottleDetectorOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class ThermalThrottleDetector extends EventEmitter {
  private opts: ThermalThrottleDetectorOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<ThermalThrottleDetectorOptions> = {}) {
    super();
