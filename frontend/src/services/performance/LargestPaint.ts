...
/**
 * LargestPaint - Performance monitoring module
 * @module services/LargestPaint
 */
export interface LargestPaintOptions {
  enabled: boolean;
  maxRetries: number;
  timeoutMs: number;
  batchSize: number;
...
export class LargestPaint extends EventEmitter {
  private opts: LargestPaintOptions;
  private active = false;
  private metrics = { calls: 0, errors: 0, latencySum: 0 };

  constructor(opts: Partial<LargestPaintOptions> = {}) {
    super();
