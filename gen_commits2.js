...
function batch(brN,dir,names){br(brN);names.forEach(nm=>{w('frontend/src/'+dir+'/'+nm+'.ts',mc(nm));C('feat: implement '+nm+' module')?n++:0;w('frontend/src/__tests__/'+dir+'/'+nm+'.test.ts',mt(nm,'../../'+dir+'/'+nm));C('test: add unit tests for '+nm)?n++:0;});w('frontend/src/'+dir+'/index.ts',names.map(x=>'export{'+x+'}from"./'+x+'";').join('\n'));C('refactor: create barrel export for '+dir)?n++:0;mg(brN);}
